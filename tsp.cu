...
__global__ void TSP_KERNELS(float* distances, unsigned int* cycle, int cities){
	int x = blockIdx.x * blockDim.x*blockDim.y + threadIdx.y*blockDim.x +threadIdx.x;
	
	// call kernels with (cycle+x) as current cycle
...
int main(int argc, char * argv[])
{
	...
	float* d_distance;
	int num_bytes = cities*cities*sizeof(float);
    cudaMalloc((void**)&d_distance,num_bytes);
    cudaMemcpy(d_distance,distance,num_bytes,cudaMemcpyHostToDevice);

    unsigned int* cycles, d_cycles;
    cycles = (unsigned int*) malloc(cities*(cities+1)*sizeof(unsigned int));
    int i=0,k;
    for(i=0;i<cities;i++){
    	k=0;
		while(k < cities){
			cycles[index(i,k,cities)] = (i+k) % cities;
			k++;
		}
		cycles[index(i,k,cities)] = i;
    }
    cudaMalloc((void**)&d_cycles, cities*(cities+1)*sizeof(unsigned int));
    cudaMemcpy(d_cycles,cycles,cities*(cities+1)*sizeof(unsigned int),cudaMemcpyHostToDevice);

    dim3 dimGrid(ceil(cities/(TILE_WIDTH*TILE_WIDTH)),1);
    dim3 dimBlock(TILE_WIDTH, TILE_WIDTH);
    TSP_KERNELS<<<dimGrid,dimBlock>>>(d_distance,d_cycles,cities);
